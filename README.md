...
**Status: pre-publish.** `withBaton` instruments an `@modelcontextprotocol/sdk` `McpServer`: wraps every tool call, injects server `instructions`, registers the `<vendor>_annotate` tool (SPEC §5.1.1–§5.1.2), injects `user_goal`/`expected_result` intent params on every wrapped tool's schema, captures a `surface_snapshot` of the vendor-true surface, and PII-scrubs every payload with the same default ruleset the Python SDK ships. What's still not here: support for the official TS SDK's **v2 packages** (`@modelcontextprotocol/server`), MRTR (Python's `_is_mrtr_pause`/`_is_mrtr_continuation`), and the low-level `Server` adapter — see [What's deferred](#whats-deferred). Not yet published to npm.
...
**The official TS SDK v2 packages.** `withBaton` targets `@modelcontextprotocol/sdk`, which is the SDK's **1.x monolith** — frozen at `1.30.0` (published 2026-07-27) though not deprecated. On 2026-07-28, the same day as the MCP 2026-07-28 spec revision, upstream split the SDK into `@modelcontextprotocol/server`, `@modelcontextprotocol/client` and `@modelcontextprotocol/core`, all at `2.0.0`, from the same `modelcontextprotocol/typescript-sdk` repository. A server built on `@modelcontextprotocol/server` is **not** instrumented by this package today: `withBaton` reaches into 1.x internals (`_registeredTools`, `server/zod-compat.js`), so a v2 server wraps to nothing — and it does so silently, which is the worse half. Real servers have already moved. Supporting v2 looks like a re-target rather than a rewrite — v2 still declares `class McpServer` with `constructor(serverInfo, options)`, `registerTool`, a private `_registeredTools`, and a zod-compat layer — but that is read off the published type declarations, not compiled against.

**MRTR handling.** Investigated 2026-08-09 and **that finding is now corrected**: it concluded "blocked upstream — nothing to duck-type against," on the strength of `npm view @modelcontextprotocol/sdk` showing `1.30.0` with no `2.0.0`, beta or `next` tag. That query was scoped to a package name the 2.0 line never used, so it could not have seen the release; the detail recorded as reassuring — 1.30.0 landing one day *before* the spec — was in fact the last release before the split. MCP 2026-07-28's MRTR mechanism (`InputRequiredResult`/`input_responses`/`request_state`) ships in Python's `mcp==2.0.0` **and** in `@modelcontextprotocol/server@2.0.0`, whose type declarations carry `InputRequired` and `requestState` throughout. So there is something to port against now; MRTR is deferred behind v2 support rather than behind upstream. Separately and still true: the *different* multi-round-trip mechanism 1.x already has — Tasks, object handlers with `createTask` — doesn't reach the code `withBaton` wraps at all. Traced through `mcp.js`'s `CallToolRequestSchema` handler, a plain-function tool (the only kind `wrapIfNeeded` wraps) always resolves as one `await handler(args, extra)` with no server-side re-invocation; only object handlers with `createTask` take the poll/resume path, and those are already skipped by `wrapIfNeeded`'s `typeof handler !== "function"` check — a separate known gap, see the module docstring in `withBaton.ts`.
