...
**Status: capture-only, pre-publish.** `withBaton` instruments an `@modelcontextprotocol/sdk` `McpServer`'s tool calls and emits Baton events. It does **not** yet do what Python's `install_baton(mcp, ...)` does beyond that — no server `instructions` injection, no `<vendor>_annotate` tool, no intent-param injection, no `surface_snapshot`. That's a deliberate scope cut, not an oversight — see [What's deferred](#whats-deferred). Not yet published to npm.
...
- `withBaton(server, config)` — wraps every tool call on an `@modelcontextprotocol/sdk` `McpServer` (peer dependency) and emits `tool_call_start` / `tool_call_end` / `tool_call_error`. Works regardless of whether tools are registered before or after `withBaton` runs (see the module docstring in `src/integrations/mcp/withBaton.ts` for why that ordering independence is load-bearing, not incidental).

`FileSink` / `MultiSink` are deliberately not here yet — see [What's deferred](#whats-deferred).

## What's deferred

Server `instructions` injection and the `<vendor>_annotate` tool are a matched pair per CHARTER §5.1.2 — instructions text exists to tell the agent to call the annotate tool, so shipping one without the other points the agent at a tool that doesn't exist. Neither is here. **This is an npm-publish blocker**, not a someday-item: don't `npm publish` this package until both land together (tracked in the `sdk-hardening` thread, `baton-internal`). Also deferred: intent-param injection (`user_goal`/`expected_result`), `surface_snapshot` capture, the low-level `Server` adapter (only the high-level `McpServer` is supported), and a real PII scrub-rule implementation (the `scrubber` hook exists; it's identity by default, same gap as core).
