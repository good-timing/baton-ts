...
1. **No attested (`h1:`) identity rung.** Python reads a principal off a verified access token's `claims["sub"]`; TypeScript's `AuthInfo` has no `claims` field, so the nearest carrier is the untyped `extra` bag and nothing specifies a subject lives there. `BatonConfig.resolveUser` (the asserted `v1:` rung) is the only identity mechanism here.
2. **Vendor hooks run INLINE with no timeout.** Python runs them off the event loop under a 5s budget (`integrations/_hooks.py`); a blocking hook here stalls its own request. Applies to `resolveUser` and `scrubber`.
3. **A malformed (lone-surrogate) `issuer` drops the issuer and still hashes**, where Python drops `user_id` entirely. The generated corpus cannot pin this — Python's generator cannot emit a vector for an input that raises.
...
- `src/identity.ts` — `user_id` hashing (`Principal`, `hashUserId`). Byte-identical to Python's `baton/identity.py`, asserted by a GENERATED corpus (`test/identityVectors.json`, regenerated by `scripts/gen_identity_vectors.py` against the Python repo). The MCP-facing half is `src/integrations/mcp/userResolution.ts`.
