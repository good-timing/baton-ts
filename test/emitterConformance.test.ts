...
const ENVELOPE_ALLOWED_TO_DIFFER = new Set([
  ...
  // The vectors were generated by Python 0.7.2, BEFORE its declared tiers
  // existed, so they record `unknown` for a client that plainly declared one.
  // Measured 2026-09-11 against the Python SDK as it ships today: the same
  // in-process round trip emits `'mcp'` — its harness client library's own
  // declared name — not `unknown`. So both SDKs now report the client that
  // declared, and the two strings differ only because the two harnesses use
...
async function runSpecScenario(): Promise<Event[]> {
  ...
  await client.callTool({
    name: "spec-vector-generator_annotate",
